...
function dodel() {
    ...
    for (var i = 0;i<$(".child:checked").length; i++) {
        ...
        $.ajax({
			url: "member57-qjwh.php",     //提交的动态页面地址
			data: 'action=check&id='+ $(".child:checked:eq("+i+")").val() ,   //提交过去的参数，运用了JS的过滤选择器 ".child:checked:eq("+i+")"表示选择类为child的且被选中的第i个
...
//-------------------------------------------------
			...
			success: function(data){
				...
				if('1'==data){
					//alert('删除成功');
					window.location.href="index.php?action=qjwh";
				}
